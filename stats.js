...
function renderCardCounts(counts) {
    ...
    const bar = segments
        .filter((s) => s.count > 0)
        .map((s) => `<div class="stats-segment ${s.color}" style="flex: ${s.count}" title="${s.label}: ${s.count}"></div>`)
        .join('');
    const legend = segments
        .map((s) => `<div class="stats-legend-item"><span class="stats-legend-dot ${s.color}"></span>${s.label} <span class="text-muted">${s.count}</span></div>`)
        .join('');
    return `
...
function renderBarChart(title, items, colorClass) {
