...
function cardState(intervalIndex) {
...
function buildBreakdown(deck, progress, keyFn) {
...
export function computeStats(deck, progress, reviewDaily, reviewLog, streak, now = Date.now()) {
    ...
    for (const card of deck) {
        ...
        counts[state]++;
        if (!p || p.dontShowUntil <= now)
            dueNow++;
    }
    const futureDueMap = new Map();
    const forecastDates = dateRange(today, 30);
    for (const d of forecastDates)
        futureDueMap.set(d, 0);
    for (const card of deck) {
        const p = progress[card.id];
        if (!p)
            continue;
