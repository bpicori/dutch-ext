...
function buildListHtml(items) {
    return items
        .map((item, i) => `<div draggable="true" data-order-idx="${i}" class="order-item flex items-center gap-sm w-full p-md rounded-lg border border-border bg-card cursor-grab active:cursor-grabbing transition-all duration-150 select-none">
...
function createOrderListController(container, initialOrder, isAnswered) {
    ...
    const refreshItemLabels = () => {
        container.querySelectorAll('.order-item').forEach((el, i) => {
            ...
            if (textEl)
                textEl.textContent = `${i + 1}. ${currentOrder[i]}`;
        });
    };
