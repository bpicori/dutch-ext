import { challengeLabel, challengeLayout, dutchPromptSentence, kbdFooter, primaryButton, } from '../ui/primitives.js';
import { applyWordOrderResult, bindChallengeSession, highlightDiff, matchesAnswer, shuffle, } from './shared.js';
const BUILT_PLACEHOLDER_HTML = '<span class="word-built-placeholder text-muted opacity-50 type-label-sm normal-case tracking-normal">Click words to build the sentence</span>';
function buildPoolHtml(wordPool) {
    return wordPool
        .map((w, i) => `<button type="button" data-word-idx="${i}" class="word-pool-btn px-md py-sm rounded-lg border border-border bg-card type-body-md text-ink transition-transform duration-150 active:scale-95 hover:border-accent hover:bg-card-hover">${w}</button>`)
        .join('');
}
function createWordPoolController(container, initialPool, isAnswered) {
    ...
    const reindexPoolButtons = () => {
        const activeButtons = [...container.querySelectorAll('.word-pool-btn')].filter((btn) => !btn.classList.contains('animate-word-out'));
    ...
    return {
        getBuiltSentence: () => {
            if (wordPool.length > 0 || wordBuilt.length === 0)
                return null;
...
function present(container, challenge) {
    ...
    return new Promise((resolve) => {
        ...
        const submit = () => {
            ...
            if (sentence)
                done({ kind: 'answer', value: sentence });
        };
