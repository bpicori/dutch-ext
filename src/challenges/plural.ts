    ...
    <div id="challenge-wrapper" class="animate-fade-in w-full flex flex-col items-center">
      <div id="challenge" class="glass-card w-full rounded-DEFAULT py-xl px-lg flex flex-col items-center gap-lg relative overflow-hidden">
        <div class="bg-surface-container-highest px-sm py-xs rounded-full border border-outline-variant">
          <span class="font-label-sm text-label-sm text-on-surface-variant tracking-[0.2em]">MEERVOUD</span>
        </div>
        <p class="font-label-sm text-label-sm text-on-surface-variant opacity-60 uppercase w-full text-center">Plural of: ${challenge.prompt}</p>
        <h1 class="font-display-word text-display-word text-primary w-full text-center lowercase">${challenge.prompt}</h1>
...
export const pluralModule: ChallengeModule = {
  present,
  showResult,
  isCorrect,
};
