import { challengeLabel, challengeShell, dutchPromptSentence, kbdFooter, } from '../ui/primitives.js';
...
function leftBtnClass(matched, active) {
    if (matched)
        return `${LEFT_BASE} ${LEFT_MATCHED}`;
    if (active)
        return `${LEFT_BASE} ${LEFT_ACTIVE}`;
...
function createMatchController(container, total) {
    ...
    const markRightTaken = (choiceIdx) => {
        ...
        if (btn)
            btn.className = rightBtnClass(true);
    };
    return {
        setActiveRow(index) {
            matchActive = index;
        ...
        pairChoice(choiceIdx) {
            if (choiceIdx < 0 || matchPairs.includes(choiceIdx))
                return false;
...
function present(container, challenge) {
    ...
    return new Promise((resolve) => {
        ...
        container.addEventListener('click', (e) => {
            ...
            if (controller.pairChoice(choiceIdx))
                finish();
        });
    });
}
function showResult(container, _challenge, _userAnswer, _correct) {
    const wrapper = container.querySelector('#challenge-wrapper');
    const matchPairs = wrapper?.dataset.matchPairs
